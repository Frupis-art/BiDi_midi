...
// Функция импорта XML (MusicXML) файлов
export const importXml = async (file: File): Promise<{ sequence1: string, sequence2: string }> => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (parseError) {
          throw new Error('Неверный формат XML файла');
        }
        ...
        // Функция для конвертации ноты из XML в наш формат
        const convertNote = (noteElement: Element): string => {
          ...
          if (rest) {
            // Это пауза
            const durationValue = duration ? parseInt(duration.textContent || '1000') : 1000;
            return durationValue !== 1000 ? `P(${durationValue})` : 'P';
          }
          ...
          // Обрабатываем альтерации (диезы/бемоли)
          if (alter) {
            ...
            if (alterValue > 0) {
              ...
              const flatNote = step + 'b';
              noteName = flatToSharp[flatNote] || noteName;
            }
          }
          ...
          // Добавляем длительность если она отличается от стандартной
          if (duration) {
            const durationValue = parseInt(duration.textContent || '1000');
            if (durationValue !== 1000) {
              noteText += `(${durationValue})`;
            }
          }
        ...
        if (parts[0]) {
          const notes = parts[0].querySelectorAll('note');
          notes.forEach(note => {
            const convertedNote = convertNote(note);
            if (convertedNote) {
              sequence1 += convertedNote;
            }
          });
        }
        
        // Обрабатываем вторую партию (если есть)
        if (parts[1]) {
          const notes = parts[1].querySelectorAll('note');
          notes.forEach(note => {
            const convertedNote = convertNote(note);
            if (convertedNote) {
              sequence2 += convertedNote;
            }
          });
        }
        
        if (!sequence1 && !sequence2) {
          throw new Error('XML файл не содержит распознаваемых нот');
        }
        
        resolve({ sequence1, sequence2 });
      } catch (error) {
        reject(new Error('Ошибка при чтении XML файла: ' + (error as Error).message));
      }
    };
